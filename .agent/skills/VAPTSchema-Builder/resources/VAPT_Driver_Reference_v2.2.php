 ...
 * Version: 2.2.0 | Bundle: 2026-02-21
 *
 * Reads vapt_driver_manifest_v2.2.json and executes each step:
 *   resolve → idempotency check → backup → insert → write → verify → rollback on failure
 *
 * Usage:
 *   $driver  = new VAPT_Driver( ABSPATH, plugin_dir_path(__FILE__) . 'vapt_driver_manifest_v2.2.json' );
...
class VAPT_Driver {
    ...
    private function execute_step( array $step, string $risk_id ): array {
        ...
        if ( ! file_exists( $target ) ) {
            $dir = dirname( $target );
            if ( ! is_dir( $dir ) ) return [ 'success' => false, 'message' => "Directory missing: {$dir}" ];
            if ( file_put_contents( $target, '' ) === false ) return [ 'success' => false, 'message' => "Cannot create: {$target}" ];
        }
        ...
        if ( ! empty( $step['backup_required'] ) ) {
            ...
            if ( ! copy( $target, $bak ) ) return [ 'success' => false, 'message' => "Backup failed: {$target}" ];
        }
    ...
    private function execute_rollback( array $step, string $risk_id ): array {
        ...
        if ( ! $target || ! file_exists( $target ) ) return [ 'success' => true, 'message' => "File not found — nothing to rollback" ];
        $content = file_get_contents( $target );
        $bm = $step['rollback']['begin_marker'] ?? $step['begin_marker'];
        $em = $step['rollback']['end_marker']   ?? $step['end_marker'];
        if ( ! str_contains( $content, $bm ) ) return [ 'success' => true, 'message' => "Marker not found — already rolled back ({$risk_id})" ];
    ...
    private function insert_block( string $content, string $block, array $insertion, string $mode ): string {
        ...
        if ( $mode === 'prepend_to_file' || $position === 'prepend' ) return $block . "\n" . $content;
        if ( $mode === 'append_to_file'  || $position === 'append'  || ! $anchor ) return rtrim( $content ) . "\n" . $block . "\n";
    ...
    private function resolve_path( string $template ): string {
        if ( str_contains( $template, '{ABSPATH}' ) ) return str_replace( '{ABSPATH}', $this->abspath, $template );
        return $template;
    }
}
 ...
 * } );
 *
 * do_action( 'vapt_apply_protection', 'RISK-003' );     // apply one risk
 * $driver->rollback( 'RISK-003' );                       // rollback one risk
 */
